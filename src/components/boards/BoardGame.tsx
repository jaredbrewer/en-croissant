  ...
  const handleHumanMove = useCallback(
    async (uci: string) => {
      ...
      try {
        ...
        if (autoFlipBoard && players.white.type === "human" && players.black.type === "human") {
          toggleOrientation();
        }
      } catch (err) {
    ...
    [gameId, gameState, autoFlipBoard, players.white.type, players.black.type, toggleOrientation],
  );
